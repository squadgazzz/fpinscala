...
object MyModule {
  ...
  def fib(n: Int): Int = {
    @tailrec def go(n: Int, a: Int, b: Int): Int = n match {
      case 0 => a
      case _ => go(n - 1, b, a + b)
    }
...
object TestSorting {
  def main(args: Array[String]): Unit = {
    ...
    val a2 = Array(1, 3, 1, 10)
    val a3 = Array(1, 10, 20, 30)
    val gt: (Int, Int) => Boolean = _ < _

    println(isSorted(a1, gt))
    println(isSorted(a2, gt))
    println(isSorted(a3, gt))
  }
}
